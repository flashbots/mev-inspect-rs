...
use ethers::{core::utils, prelude::JsonRpcClient, providers::{FromErr, Middleware}, types::{BlockNumber, Trace}};
use serde::{de::DeserializeOwned, Serialize};
use std::path::{Path, PathBuf};
use log::*;

#[derive(Clone, Debug)]
pub struct CachedBorProvider<M> {
    inner: M,
    cache: PathBuf,
}
...
impl<M: Middleware> CachedBorProvider<M> {
...
#[async_trait]
impl<M: Middleware> Middleware for CachedBorProvider<M> {
    ...
    async fn trace_transaction(
        &self,
        hash: ethers::prelude::TxHash,
    ) -> Result<Vec<Trace>, Self::Error> {
        error!("Called here: Tracing override");
        Ok(vec![])
        // let hash = utils::serialize(&hash);
        // self.inner().provider().request(method, params)
        // let a = self.inner().provider();
        // self.inner().provider()
        // self.inner().provider().clone().request("debug_traceTransaction", vec![hash]).await
    }
}
